...
export default async function() {
    ...
    if(!isAuthorized) {
        ...
        const allInfo = await getPostAndComments()

        addToolbarAndPostHtml(userData, allInfo)
        if(allInfo.commentsInfo) {
            addCommentsHtml(allInfo)
        }
...
const getPostAndComments = async () => {
    try {
        const response = await fetch('/get-post-details')
        if (response.ok) {
...
const addCommentsHtml = (allInfo) => {
    allInfo.commentsInfo.forEach(comment => {
        ...
        <div class="postbox">
            ...
            <div class="reactionbox">
                <button class="reactionbtn" id="commentLike" value="${comment.commentId}">👍</button>
                <a style="color: green; text-shadow: 1px 1px 0 #000; font-size: 30px;">${comment.likes}</a>
                <button class="reactionbtn" id="commentDislike" value="${comment.commentId}">👎</button>
